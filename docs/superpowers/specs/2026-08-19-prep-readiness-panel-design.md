...
4. **Cardio/posing completion.** The existing `cardioCount`/`posingCount` for the week, displayed alongside the most recent `cardio_rx` free-text prescription from the latest weekly decision. No fabricated numeric target — the Rx is coach-written free text today, so this stays a display pairing, not a red/green pass/fail (a structured cardio/posing target is a real future upgrade, not invented here).
