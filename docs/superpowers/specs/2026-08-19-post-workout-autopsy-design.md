...
1. **Rx vs. actual.** One overall-session badge — beat / met / missed — computed the same way `GymDebriefLogic.formatRxComparison()` already does (comparing `getRx(ex, priorLogs)` against today's logged sets for each exercise), collapsed to a single line so a normal beat/met session stays a one-glance read. When the overall result is "missed," the specific exercise name(s) that fell short are named in the "why it changed" prompt below, rather than showing every exercise's own badge — keeps the fast path fast, still names the actual problem when there is one. No LLM call.
